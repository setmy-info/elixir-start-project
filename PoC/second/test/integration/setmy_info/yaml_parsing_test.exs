defmodule SetmyInfo.Integration.YamlParsingTest do
  @moduledoc """
  Integration tests for YAML parsing via `SetmyInfo.YamlParser`.
  ...
  describe "parse_file/1 — config.yml" do
    test "loads calculator application configuration from file" do
      path = Path.join(@fixtures_dir, "config.yml")

      assert {:ok, config} = YamlParser.parse_file(path)

      app = config["app"]
      assert app["name"] == "calculator-app"
      ...
      assert flags["swagger_ui"] == true
      assert flags["graphiql"] == true
      assert flags["mutation_testing"] == false
    end
  end
  ...
  describe "parse_file/1 — persons.yml (domain object hydration)" do
    ...
    test "parsed persons can be mapped to plain Elixir structs" do
      path = Path.join(@fixtures_dir, "persons.yml")
      assert {:ok, %{"persons" => persons}} = YamlParser.parse_file(path)
      ...
      assert Enum.all?(records, fn r -> is_binary(r.first_name) and is_integer(r.age) end)
    end
  end
