defmodule SetmyInfo.Lessons.DataTypesTest do
  ...
  describe "Nil" do
    test "nil is the absence of a value" do
      ...
      IO.puts("is_nil(nil)   => #{DataTypes.nil?(nil)}")
      IO.puts("is_nil(false) => #{DataTypes.nil?(false)}")
      IO.puts("is_nil(0)     => #{DataTypes.nil?(0)}")
      IO.puts("is_atom(nil)  => #{is_atom(nil)}")

      assert DataTypes.nil_example() == nil
      assert DataTypes.nil?(nil) == true
      assert DataTypes.nil?(false) == false
      assert DataTypes.nil?(0) == false
    end
  end
