defmodule CalculatorRest.Router do
  use Plug.Router
  ...
  Static files are served from the project-level `web-app` folder on the same port.
  """

  forward("/api/graphql",
    to: Absinthe.Plug,
    init_opts: [schema: CalculatorRest.Schema]
  )

  forward("/graphiql",
    to: Absinthe.Plug.GraphiQL,
    init_opts: [schema: CalculatorRest.Schema, interface: :simple, json_codec: Jason]
  )

  import Plug.Conn

  alias CalculatorRest.Swagger
  alias Math.MathService

  @web_app_dir Path.expand("../../web-app", __DIR__)
  ...
  get "/" do
    index_path = Path.join(@web_app_dir, "index.html")
